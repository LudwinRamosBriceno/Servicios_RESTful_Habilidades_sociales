...
def _connect() -> pika.BlockingConnection:
    """Establece una conexión con RabbitMQ usando la URL configurada."""
...
def publish_event(
    ...
    *,
    ...
    Incluye tipo de evento, datos, versión y un ID de correlación opcional. El evento se publica en el intercambio definido por EXCHANGE_NAME usando el tipo de evento como clave de enrutamiento.
    """
    event = {
...
def start_consumer(
    ...
    def _run() -> None:
        """
        Función interna que consume mensajes de RabbitMQ en un hilo separado.

        Declara el intercambio y la cola, se suscribe a los mensajes y llama al handler.
        """
        while True:
            ...
            try:
                ...
                def _on_message(ch, method, _properties, body) -> None:
                    """Maneja un mensaje recibido y lo procesa con el handler."""
                    try:
