"""Servicio de productos para gestionar catalogo e inventario."""
...
class ProductService:
    """Servicio de productos para gestion e inventario."""

    def __init__(self, repository: ProductRepository) -> None:
        """Inicializa el servicio de productos."""
        self._repository = (
            repository  # instancia del repositorio para acceder a la base de datos
        )
    ...
    def get_product(self, product_id: str) -> ProductResponse:
        """Obtiene un producto por su id o lanza HTTP 404."""
    ...
    def create_product(self, payload: CreateProductRequest) -> ProductResponse:
        """Crea un producto o actualiza si el ID ya existe."""
    ...
    def update_product(
        ...
        """Actualiza un producto por su id o lanza HTTP 404."""
        product = self._repository.find_by_id(product_id)
        if not product:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Producto no encontrado"
    ...
    def delete_product(self, product_id: str) -> None:
        """Elimina un producto por su id o lanza HTTP 404."""
        product = self._repository.find_by_id(product_id)
        if not product:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Producto no encontrado"
    ...
    def discount_stock(
        self, product_id: str, payload: UpdateStockRequest
    ) -> ProductResponse:
        """Descuenta stock o lanza HTTP 404/422 segun corresponda."""
        product = self._repository.find_by_id(product_id)
        if not product:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Producto no encontrado"
    ...
    def _to_response(product: Product) -> ProductResponse:
        """Convierte un producto a respuesta para el cliente."""
    ...
    def _seed_products(self) -> None:
        """Carga productos de prueba si no existen."""
        try:
            ...
            logger.warning(
                "Products table is missing. Skipping seed until migrations are applied."
            )
            return
        ...
        seed_data = [
            Product(
                id="hab_001",
                name="empatia",
                description="Comprender emociones ajenas",
                stock=100,
                active=True,
            ),
            Product(
                id="hab_002",
                name="amistad",
                description="Crear vinculos positivos",
                stock=100,
                active=True,
            ),
            Product(
            ...
            Product(
                id="hab_004",
                name="creatividad",
                description="Generar ideas nuevas",
                stock=90,
                active=True,
            ),
            Product(
                id="hab_005",
                name="resiliencia",
                description="Superar adversidad",
                stock=70,
                active=True,
            ),
            Product(
                id="hab_006",
                name="comunicacion",
                description="Transmitir ideas claramente",
                stock=100,
                active=True,
            ),
            Product(
                id="hab_007",
                name="colaboracion",
                description="Trabajar en equipo",
                stock=110,
                active=True,
            ),
            Product(
                id="hab_008",
                name="sagacidad",
                description="Percibir con agudeza",
                stock=60,
                active=True,
            ),
            Product(
            ...
            Product(
                id="hab_010",
                name="respeto",
                description="Valorar a los demas",
                stock=120,
                active=True,
            ),
            Product(
                id="hab_011",
                name="confianza",
                description="Seguridad personal y social",
                stock=90,
                active=True,
            ),
            Product(
                id="hab_012",
                name="humor",
                description="Usar humor apropiado",
                stock=100,
                active=True,
            ),
            Product(
                id="hab_013",
                name="adaptabilidad",
                description="Ajustarse al cambio",
                stock=80,
                active=True,
            ),
            Product(
                id="hab_014",
                name="escucha activa",
                description="Escuchar con atencion",
                stock=95,
                active=True,
            ),
            Product(
                id="hab_015",
                name="iniciativa",
                description="Actuar de forma proactiva",
                stock=85,
                active=True,
            ),
        ]
